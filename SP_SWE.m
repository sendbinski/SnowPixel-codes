...
METHOD = 2; %%%% 1 or 2. 1 for precip particle "id" (different threshold really).   

%%%%%%%%% Method 1
precip_threshold = 10; % mW*mW on the "enhanced" chart.

%%%%%%%%% Method 2
quant = 0.98; % percentage for quantile separation between wind and precip. between 0 and 1
...
% %%% Might not need void ratio with good cleaning %%%
% void_ratio = 2.0467; %ratio between total pixels inside the castle walls and pixels that are Not NaNs
% void_ratio = 1; %%%%% accounts for pixels within the castle walls that are nans but not dead, ie all over the place ~1.1 %%%%%%%%%
...
swe_rate_thresh = 0.4; %mm/hr, truncates SP data below this value to zero to prevent erroneous growth
Freq = 10; %Hz
Lv = 2256.4e3; % J/kg water
Lf = 334e3; %J/kg water
Leq = Lv + Lf; %Lv + Lf for snow. Lv only for rain
c = 4.18e3; %cp J/kgK
dT = 105; %C
...
block_idx=0;
block_int_mJ = double.empty(0,num_blocks);
...
if METHOD == 1
    ...
    enhanced = block(2:end-1,:).*(0.5*(block(1:end-2,:) + block(3:end,:)));
   
    precip_threshold(i) = quantile(enhanced,quant,"all");
    mode_mean = nanmean(block(enhanced<=precip_threshold(i))); % mean of data below threshold
    block_subt = block-mode_mean; % subtr

    block_int_mJ(i) = nansum(nansum(block_subt));  
...
if METHOD == 2
    ...
    %%% Threshold method
    thresh(i) = quantile(block,quant,"all");
    mode_mean = nanmean(block(block<=thresh(i))); % mean of data below threshold
    block_subt = block-mode_mean; % subtract mode mean from total signal
    block_int_mJ(i) = nansum(nansum(block_subt)); %mW to mJ "integration" summation identical to trapz
...
block_int_J=block_int_mJ'*mJtoJ; % mJ to J
block_int_J_sec = block_int_J/Freq; % divide 10 Hz data by 10 Hz to get 1 Hz

kilograms=block_int_J_sec/(Leq + c*dT); % J to kg 


volume = kilograms/rho; % kg to m^3
height_m=volume/sensor_area; % m^3 to m
swe_in_mm = height_m*mtomm; % m to mm



swe_rate = swe_in_mm/(hour_fraction);

new_swe_rate = (swe_rate.*(swe_rate>swe_rate_thresh)); % adjusting for low swe rates
new_swe_in_mm = new_swe_rate*hour_fraction;
...
if METHOD == 1
    ...
    %%%% SWE ACCUM
    figure;plot(hour_fraction:hour_fraction:length(swe_in_mm)*hour_fraction,cumsum(swe_in_mm-min(swe_in_mm)),'LineWidth',2) %%%%%%%   multiplies by void ratio. Funny indexing is to align time stamps
    hold
    ...
    set(gca,'fontsize',16)
    legend('SP','DEID','Location','best','box','off')
    title(['Threshold = ' num2str(mean(precip_threshold)) ' mW*mW, ' num2str(num_pix) 'pix'])
end
...
if METHOD == 2
    ...
    hold
    plot(hour_fraction:hour_fraction:length(deidswe_jan_6_fivemin)*hour_fraction,deidswe_jan_6_fivemin/(hour_fraction),'LineWidth',1)
    xlabel('Time (hours)')
    ylabel('swe rate (mm/hr)')
    set(gca,'fontsize',16)
    legend('SP','DEID','Location','best','box','off')
    title(['Percentile = ' num2str(quant*100) ', ' num2str(swe_rate_thresh) ' mm/hr, medianbounds = ' num2str(med_lower) ' ' num2str(med_upper)])
    %%%% SWE ACCUM
